...
class CartService extends BaseService
{
     ...
     * 清空购物车
     */
    public function clearCartGoods($userId, $cartId = null): ?bool
    {
        if (empty($cartId)) {
    ...
    {
        $grouponRules    = GrouponService::getInstance()->getGrouponRuleById($grouponRulesId);
        $checkGoodsPrice = 0;
     ...
     * @param $userId
     * @param null $cartId
     * @return Cart|Cart[]|Builder|Builder[]|Collection|Model|object|null
    ...
    public function getValidCartList($userId)
    {
        ...
        $inValidCartIds = [];
        $goodsList      = GoodsService::getInstance()->getGoodsListByIds($goodIds)->keyBy('id');
    ...
    public function getGoodInfo($goodsId, $productId)
    {
        ...
        $goodProduct = GoodsService::getInstance()->getGoodsProductById($productId);
     ...
     */
    public function editCartNum($cartProduct, $goodProduct, $num): ?Cart
    {
    ...
    public function add($goodsId, $productId, $number, $userId)
    {
        ...
        $cartProduct = CartService::getInstance()->getCartProduct($userId, $productId, $goodsId);

        if (is_null($cartProduct)) {
            return CartService::getInstance()->newCart($userId, $goodProduct, $goods, $number);
        } else {
     ...
     * add cart
     */
    public function newCart($userId, GoodsProduct $goodsProduct, Goods $goods, $number): Cart
    {
        $cart = new Cart();
